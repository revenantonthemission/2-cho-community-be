...
flowchart TD
    ...
    subgraph DB["MySQL"]
        Tables["38개 테이블<br/>user, post, comment, notification,<br/>tag, poll, dm_conversation, social_account,<br/>wiki_page, wiki_page_tag, wiki_page_revision,<br/>reputation_event ..."]
    end
...
`core/utils/email.py`에서 `EMAIL_BACKEND` 설정에 따라 SES(프로덕션) 또는 SMTP(로컬) 사용. `asyncio.to_thread()`로 블로킹 SMTP I/O를 비동기 처리.

### @멘션 알림

`core/utils/mention.py`의 `extract_mentions()`가 게시글/댓글 본문에서 `@닉네임` 패턴(`[a-zA-Z0-9_]{3,10}`)을 파싱. 정규식은 `modules/user/schemas.py`의 `_NICKNAME_PATTERN`과 동일 문자셋. 수정 시에는 기존 멘션과의 차집합으로 **새로 추가된 멘션에 대해서만** 알림 발송.

### 이미지 리사이징

`core/utils/image_resize.py`에서 Pillow를 이용해 업로드 시 자동 리사이징. 프로필 최대 400x400, 게시글 최대 폭 1200px (비율 유지). GIF는 애니메이션 보존을 위해 리사이징 제외. local/S3 양쪽 스토리지에 동일 적용.
...
`core/utils/websocket_pusher.py`가 알림 생성 시 Redis Pub/Sub(K8s) 또는 로컬 인메모리 맵(DEBUG)으로 실시간 알림 전송. Best-effort 방식으로 트랜잭션 밖에서 처리되며, 실패해도 알림 생성에 영향 없음.

### datetime 포맷팅

모든 모델의 datetime 필드는 `core/utils/formatters.py`의 `format_datetime()`으로 ISO 8601 변환 후 반환. 모델 레벨에서 일관 적용.
...
# 시딩 실행
python core/database/seed_data_large.py \
