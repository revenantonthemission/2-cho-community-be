...
async def create(
    ...
    async with transactional() as cur:
        await cur.execute(
            "INSERT INTO social_account (user_id, provider, provider_id, provider_email) VALUES (%s, %s, %s, %s)",
...
async def get_by_user_id(user_id: int) -> list[dict]:
    ...
    async with get_cursor() as cur:
        await cur.execute(
            "SELECT id, provider, provider_id, provider_email, created_at FROM social_account WHERE user_id = %s",
            (user_id,),
        )
