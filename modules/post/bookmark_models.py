...
async def get_bookmark(post_id: int, user_id: int) -> Bookmark | None:
    ...
    async with get_cursor() as cur:
        await cur.execute(
            "SELECT id, user_id, post_id, created_at FROM post_bookmark WHERE post_id = %s AND user_id = %s",
            (post_id, user_id),
        )
        row = await cur.fetchone()
        return Bookmark(**row) if row else None
...
async def get_post_bookmarks_count(post_id: int) -> int:
    ...
    async with get_cursor() as cur:
        await cur.execute(
            "SELECT COUNT(*) AS cnt FROM post_bookmark WHERE post_id = %s",
            (post_id,),
        )
        row = await cur.fetchone()
...
async def add_bookmark(post_id: int, user_id: int) -> Bookmark:
    ...
    async with transactional() as cur:
        await cur.execute(
            "INSERT INTO post_bookmark (user_id, post_id) VALUES (%s, %s)",
            (user_id, post_id),
        )
        bookmark_id = cur.lastrowid

        await cur.execute(
            "SELECT id, user_id, post_id, created_at FROM post_bookmark WHERE id = %s",
            (bookmark_id,),
        )
        row = await cur.fetchone()
...
async def remove_bookmark(post_id: int, user_id: int) -> bool:
    ...
    async with transactional() as cur:
        await cur.execute(
            "DELETE FROM post_bookmark WHERE post_id = %s AND user_id = %s",
            (post_id, user_id),
        )
