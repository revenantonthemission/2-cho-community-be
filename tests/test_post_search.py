...
async def test_search_posts_by_title(authorized_user):
    ...
    # 게시글 3개 생성 (제목이 서로 다름)
    await client.post("/v1/posts/", json={"title": "파이썬 입문 가이드", "content": "내용1"})
    await client.post("/v1/posts/", json={"title": "자바스크립트 튜토리얼", "content": "내용2"})
    await client.post("/v1/posts/", json={"title": "파이썬 고급 패턴", "content": "내용3"})
...
async def test_search_posts_by_content(authorized_user):
    ...
    await client.post(
        "/v1/posts/",
        json={"title": "일반 제목", "content": "데이터베이스 최적화 전략에 대해 알아봅시다"},
    )
    await client.post(
        "/v1/posts/",
        json={"title": "다른 제목", "content": "프론트엔드 개발 이야기"},
    )
...
async def test_search_empty_keyword_returns_all(authorized_user):
    ...
    await client.post("/v1/posts/", json={"title": "제목1", "content": "내용1"})
    await client.post("/v1/posts/", json={"title": "제목2", "content": "내용2"})
...
async def test_search_no_results(authorized_user):
    ...
    await client.post("/v1/posts/", json={"title": "일반 게시글", "content": "일반 내용"})
...
async def test_sort_by_latest(authorized_user):
    ...
    res1 = await client.post("/v1/posts/", json={"title": "첫번째 글", "content": "내용"})
    res2 = await client.post("/v1/posts/", json={"title": "두번째 글", "content": "내용"})
...
async def test_sort_by_views(authorized_user):
    ...
    await client.post("/v1/posts/", json={"title": "조회수 적은 글", "content": "내용"})
    res2 = await client.post("/v1/posts/", json={"title": "조회수 많은 글", "content": "내용"})
...
async def test_sort_invalid_fallback_to_latest(authorized_user):
    ...
    await client.post("/v1/posts/", json={"title": "테스트 글", "content": "내용"})
...
async def test_search_with_sort(authorized_user):
    ...
    res1 = await client.post(
        "/v1/posts/", json={"title": "FastAPI 기초", "content": "기초 내용"}
    )
    res2 = await client.post(
        "/v1/posts/", json={"title": "FastAPI 심화", "content": "심화 내용"}
    )
    await client.post(
        "/v1/posts/", json={"title": "Django 입문", "content": "다른 프레임워크"}
    )
...
async def test_search_whitespace_only_returns_all(authorized_user):
    ...
    await client.post("/v1/posts/", json={"title": "테스트 게시글", "content": "내용"})
...
async def test_search_fulltext_special_chars(authorized_user):
    ...
    await client.post("/v1/posts/", json={"title": "특수문자 테스트", "content": "내용 내용"})
