...
from tests.conftest import create_verified_user, create_test_comment
...
async def test_mark_notification_as_read(client, two_users_with_post):
    ...
    res = await client.patch(
        f"/v1/notifications/{notification_id}/read", headers=author["headers"],
    )
    ...
    noti = next(
        n for n in list_res2.json()["data"]["notifications"]
...
async def test_mark_all_notifications_as_read(client, two_users_with_post):
    ...
    # Act
    res = await client.patch(
        "/v1/notifications/read-all", headers=author["headers"],
    )
...
async def test_unread_count_returns_correct_number(client, two_users_with_post):
    ...
    res = await client.get(
        "/v1/notifications/unread-count", headers=author["headers"],
    )
...
async def test_mark_other_user_notification_returns_403_or_404(
    client, fake, two_users_with_post,
):
