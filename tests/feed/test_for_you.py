...
async def test_for_you_with_scores_uses_score_ordering(client, fake):
    ...
    # 게시글 2개 생성
    post_low = await create_test_post(
        client, user["headers"], title="낮은 점수 게시글"
    )
    post_high = await create_test_post(
        client, user["headers"], title="높은 점수 게시글"
    )
    ...
    async with get_connection() as conn:
        async with conn.cursor() as cur:
            await cur.execute(
                "INSERT INTO user_post_score (user_id, post_id, combined_score) "
                "VALUES (%s, %s, %s), (%s, %s, %s)",
                (
                    user["user_id"], post_low["post_id"], 10.0,
                    user["user_id"], post_high["post_id"], 90.0,
                ),
            )
